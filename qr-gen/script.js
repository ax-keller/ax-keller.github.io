...
const crcTable = [
  ...
  0xD80D2BDA, 0xAF0A1B4C, 0x36034AF6, 0x41047A60, 0xDF60EFC3, 0xB865DF55, 0x316E8EEF, 0x4669BE79,
...
function bytes(num, byteLength) {
  ...
  return binStr.match(/\d{8}/g).map(v => parseInt(v, 2));
}

function writePngDpi(arrayBuffer, dpi) {
  const byteArray = new Uint8Array(arrayBuffer);
  if (byteArray[0] !== 0x89 || byteArray[1] !== 0x50) return byteArray;
  
  let ptr = 8;
  while (ptr < byteArray.length) {
    const chunkLength = (byteArray[ptr] << 24) | (byteArray[ptr + 1] << 16) | (byteArray[ptr + 2] << 8) | byteArray[ptr + 3];
    const type = String.fromCharCode(byteArray[ptr + 4], byteArray[ptr + 5], byteArray[ptr + 6], byteArray[ptr + 7]);
    
    if (type === 'IDAT') {
      const PX_PER_METER = 2835;
      const dpr = dpi / 72;
      const ppm = Math.floor(PX_PER_METER * dpr);
      const data = [...bytes(ppm, 4), ...bytes(ppm, 4), 1];
      const typeBytes = [0x70, 0x48, 0x59, 0x73];
      const crcVal = crc([...typeBytes, ...data]);
      const pHYsChunk = [...bytes(9, 4), ...typeBytes, ...data, ...bytes(crcVal, 4)];
      
      const result = new Uint8Array(byteArray.length + pHYsChunk.length);
      result.set(byteArray.slice(0, ptr), 0);
      result.set(pHYsChunk, ptr);
      result.set(byteArray.slice(ptr), ptr + pHYsChunk.length);
      return result;
    }
...
async function download() {
  ...
  const ppi = parseInt(ppiInput.value, 10) || 96;
  const filename = `qr_output.${format}`;
  ...
  if (format === "svg") {
    ...
    img.onload = async () => {
      ...
      const newBuf = writePngDpi(buf, ppi);
      const finalBlob = new Blob([newBuf], { type: "image/png" });
...
async function copyToClipboard() {
  ...
  try {
    if (format === "svg") {
      ...
      const item = new ClipboardItem({ "image/png": blobOut });
      await navigator.clipboard.write([item]);
    }
